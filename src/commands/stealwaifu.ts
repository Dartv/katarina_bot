...
const getStolenCharacters = async (...ids: ObjectId[]): Promise<[ICharacter, ICharacter[]]> => {
  const query = {
    _id: { $in: ids },
  };
...
const handler = async (context) => {
  try {
    ...
    const mentionedMember = await message.mentions.members.first();

    if (!mentionedMember) return null;

    const mentionedUser: any = await User.findOne({ discordId: mentionedMember.id });
