...
describe('expectRefToBeUnique', () => {
  ...
  it('should dispatch an error when the image with given ref already exists', async () => {
    ...
    const errorResponseA = await expectRefToBeUnique(
      R.view(lenses.user.images as any)
    )(next, context);
    const errorResponseB = await expectRefToBeUnique(
      R.view(lenses.guild.images as any)
    )(next, context);
  ...
  it('should successfully pass through when the image with given ref isn\'t found', async () => {
    ...
    const nextContext = await expectRefToBeUnique(
      R.view(lenses.user.images as any)
    )(next, context);
